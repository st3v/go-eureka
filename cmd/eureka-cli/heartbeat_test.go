...
var _ = Describe("heartbeat command", func() {
	var (
		server      *ghttp.Server
		instanceXml []byte
		xmlPath     string
	...
	BeforeEach(func() {
		...
		args = []string{"heartbeat", "-i", xmlPath, "-e", server.URL()}
		...
		server.AppendHandlers(
			ghttp.CombineHandlers(
				ghttp.VerifyRequest("PUT", "/apps/myapp/id"),
	...
	It("provides basic logs on stdout", func() {
		Eventually(session).Should(gbytes.Say("Sending heartbeat for instance 'id' of application 'myapp'..."))
		Eventually(session).Should(gbytes.Say("Success"))
	})

	Context("when the --instance flag has not been specified", func() {
		BeforeEach(func() {
			args = []string{"heartbeat", "-e", server.URL()}
		})
	...
	Context("when no --endpoint flag has not been specified", func() {
		BeforeEach(func() {
			args = []string{"heartbeat", "-i", xmlPath}
		})
	...
	Context("when the instance file does not exist", func() {
		BeforeEach(func() {
			args = []string{"heartbeat", "-i", "/path/to/nowhere", "-e", server.URL()}
		})
	...
	Context("when the instance file does not specifiy an instance", func() {
		BeforeEach(func() {
			...
			args = []string{"heartbeat", "-i", path, "-e", server.URL()}
		})
